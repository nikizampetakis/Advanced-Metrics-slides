...
* Will use the [`feols`](https://www.rdocumentation.org/packages/fixest/versions/0.8.4/topics/feols) function from the `fixest` package.

* We will compute *robust* standard errors which are correcting for heteroskedasticity. [Details here.](https://cran.r-project.org/web/packages/fixest/vignettes/standard_errors.html)
]
...
tab = modelsummary(models = mod,
                       stars = TRUE,
                       gof_omit = 'Std.Errors|DF|Deviance|AIC|BIC|R2 Adj.|p.value|se_type|statistic|Log.Lik.|Num.Obs.|N|RMSE',
         )
tab
```
]
...
# add exogenous vars!
mod[["2SLS_yr"]] <- feols(lnw ~ yob_fct | s ~ q4, data = ak91, vcov = "hetero")  
# use all quarters as IVs
mod[["2SLS_all"]] <- feols(lnw ~ yob_fct | s ~ qob_fct, data = ak91, vcov = "hetero")
```
...
tab = modelsummary(models = mod[c("ols","2SLS","ols_yr","2SLS_yr","2SLS_all")],
                   stars = TRUE,
                   statistic = 'std.error',
                   gof_omit = 'Std.Errors|DF|Deviance|AIC|BIC|R2 Adj.|p.value|F$|se_type|statistic|Log.Lik.|Num.Obs.|N|RMSE',
                   add_rows = rows,
                   coef_omit = 'yob_fct')
...
mods$IV  <- feols(lwage ~ 1 | educ ~ fatheduc, data = mroz, vcov = "hetero")
```
...
bind_rows(
  ...
  coefs_IV %>% filter(term == "fit_educ") %>% mutate(estimator = "IV")
  ) %>%
