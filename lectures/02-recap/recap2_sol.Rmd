...
library(pacman)
p_load(broom, latex2exp, ggplot2, ggthemes,  ggforce, viridis, dplyr, magrittr, knitr, parallel, wooldridge)
# Define pink color
scpo_red <- "#d90502"
# Knitr options
opts_chunk$set(
...
.pull-right[
```{r,echo=FALSE, dev = "png", dpi = 50, out.width = "650px", cache = F,eval = T}
...
2. The key is to have a mental map of what *should* belong to the model. 
...
2. Run `skimr::skim` on the dataset to get an overview. what is the fraction of nonwhite in the data?
...
annotate(
  ...
  label = TeX("$\\hat{\\beta}_0 + \\hat{\\beta}_1 = Group_1$"),
  size = 7,
  color = scpo_red
) +
theme_empty
...
```{R, gen dataset, echo = FALSE, cache = T, results = "hide"}
